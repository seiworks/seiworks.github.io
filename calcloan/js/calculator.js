(function () {
    ...
    function createScenario({ id, name, housePrice, ltv, qingan, standard, fee = 0 }) {
        const totalLoan = housePrice * ltv;
        const cash = housePrice - totalLoan;
        const loans = [];
        const qinganPrincipal = Math.min(totalLoan, qingan.limit);
        const standardPrincipal = totalLoan - qinganPrincipal;

        if (qinganPrincipal > 0) {
            loans.push({
                name: '新青安',
                ...buildLoanSchedule(qinganPrincipal, qingan.rate, qingan.years, qingan.grace)
            });
        }
        if (standardPrincipal > 0) {
            loans.push({
                name: '一般房貸',
                ...buildLoanSchedule(standardPrincipal, standard.rate, standard.years, standard.grace)
            });
        }
    ...
    form.addEventListener('submit', (event) => {
        ...
        const qinganLimit = getNumber('#qingan-limit');
        ...
        for (const offer of offers) {
            const qingan = {
                limit: qinganLimit,
                years: getPlainNumber('.qingan-years', offer),
                grace: getPlainNumber('.qingan-grace', offer),
                rate: getPlainNumber('.qingan-rate', offer)
            };
            const standard = {
            ...
            if (qingan.grace >= qingan.years || standard.grace >= standard.years) {
